...
State Driven Views are very useful elements - they allow developers to control what is being rendered depending on the state of the application or state of the middleware components. not only from the perspective of the code complexity, but also when you look at them as the objects to test. You reduce the possibility to mess up with the state (because you have only one component instead of the number of them), they help you keep your code concise and clean, and of course to ship a good quality code faster because you reduce the number of redundant elements. State Driven Views allow you to build additional paths in your business logic with conditional rendering, middleware and components composition that you can all do with React.
